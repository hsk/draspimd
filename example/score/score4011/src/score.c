...
static void preScoreDraw(register u32 score,u16* src) {
    s32 j=7;
    __asm("rol.l #8,%0":"=r" (score):"rw"(score));
    for(;j;j--) {
      if(score & 0xf) break;
      *src++ = 0;
      __asm("rol.l #4,%0":"=r" (score):"rw"(score));
    }
    for(;j;j--) {
      *src++ = ((SCORE_PAL << 13) | (1 << 15) | 0x580 + '0') + (score & 0xf); 
      __asm("rol.l #4,%0":"=r" (score):"rw"(score));
    }
}

static void scoreDrawTop1(u32* score) {
  *(vu32*)0xC00004 = GFX_WRITE_VRAM_ADDR(SCORE_VDP_PLAN + ((TOP_SCORE_X + (TOP_SCORE_Y << 6)) <<1));
  *(vu32*)0xC00000 = score[0] ^ ((2+(2<<16))<<13);
  *(vu32*)0xC00000 = score[1] ^ ((2+(2<<16))<<13);
  *(vu32*)0xC00000 = score[2] ^ ((2+(2<<16))<<13);
  *(vu16*)0xC00000 = *(vu16*)&score[3] ^ (2<<13);
}
static void scoreDrawScore1(u32* score) {
    *(vu32*)0xC00004 = GFX_WRITE_VRAM_ADDR(SCORE_VDP_PLAN + ((SCORE_X + (SCORE_Y << 6)) <<1));
    *(vu32*)0xC00000 = score[0];
    *(vu32*)0xC00000 = score[1];
    *(vu32*)0xC00000 = score[2];
    *(vu16*)0xC00000 = *(vu16*)&score[3];
}
void score_update() {
  u16 sc[8];
  preScoreDraw(score,sc);
  scoreDrawScore1(sc);
  if (top_score<score) scoreDrawTop1(sc);
  //harriers_update();
}
